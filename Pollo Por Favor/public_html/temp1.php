<?php
   ...
   $ses_dinner = mysqli_query($con,"select * from menu_items where food_type = 'Beverage' ");
...
<html>
    ...
    <body>
		...
		<h2> 
			...
			?> </font>
			<br>
                        Beverage: <font color="black">
                        <?php
                            for( $row= mysqli_fetch_row($ses_beverage); $row != false; $row = mysqli_fetch_row($ses_dessert)) {
                                printf( "%s", $row[1]);
                                echo "<br>";
                            }
                            ?> </font>
